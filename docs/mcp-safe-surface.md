...
| `journal_get_capabilities` | Read-only | `GET /meta/capabilities` | `{}` | Versioned protocol, feature, schema, transport, tool, and actual bounded-limit support |
...
- Capability discovery is authenticated like the other ledger routes and independent of ledger
  contents. Unknown stable feature identifiers are safe to ignore; protocol support must not be
  confused with the assurance of a client-reported verification record.
